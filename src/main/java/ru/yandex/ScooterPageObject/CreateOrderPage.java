...
public class CreateOrderPage {
     ...
     * Используйте 0, если нужна первая кнопка Заказать, 1, если вторая и тд. */
    ...
    /** Метод проверяет, что в случае некорректного взаимодействия с полем Имя,
     * появляется ошибка */
    ...
    /** Метод проверяет, что в случае некорректного взаимодействия с полем Фамилия,
     * появляется ошибка */
    ...
    */
    ...
    /** Метод проверяет, что в случае некорректного взаимодействия с полем Адрес,
     * появляется ошибка */
    ...
    */
    ...
    /** Метод проверяет, что в случае некорректного взаимодействия с полем Метро,
     * появляется ошибка */
    ...
    /** Метод проверяет, что в случае некорректного взаимодействия с полем Телефон,
     * появляется ошибка */
    ...
    /**
     * Строка, по клику на которую открывается дата пикер.
     * Метод за счет использования get позволяет выбрать нужную дату
     ...
     * Открывает модальное окно в котором можно окончательно
     * подтвердить или отклонить завершение Заказа
     */
     ...
     * Кнопка доступна после нажатия кнопки - Заказать. Имеется ввиду не та
     * кнопка, что вверху страницы. Сама кнопка внутри этого метода закрывает
     * модальное окно, которое запрашивает подтверждение заказа
     */
     ...
     * Кнопка доступна после нажатия кнопки - Заказать. Имеется ввиду не та
     * кнопка, что вверху страницы. Сама кнопка внутри этого метода закрывает
     * модальное окно и завершает заказ. Далее выводиться модалка успешно
     * совершенного заказа
     */
     ...
     */
    public CreateOrderPage chekOrderCreatedSuccessful() {
